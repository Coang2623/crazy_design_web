// src/pages/Admin.jsx — Phase 3 refactor with CMS tabs
...
const SIDEBAR_TABS = [
    { id: 'bookings', icon: 'calendar_month', label: 'Lịch hẹn' },
    { id: 'content', icon: 'edit_note', label: 'Nội dung' },
    { id: 'testimonials', icon: 'star', label: 'Đánh giá' },
    { id: 'pricing', icon: 'payments', label: 'Bảng giá' },
];

// ─── Sub-components ──────────────────────────────────────────────
function StatusBadge({ status }) {
    const s = STATUS_STYLES[status] || STATUS_STYLES.pending;
    return <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${s.color}`}>{s.label}</span>;
}

function StatCard({ icon, label, value, color }) {
    return (
        <div className="bg-white dark:bg-dark-900 rounded-2xl p-5 border border-gray-100 dark:border-dark-800 flex items-center gap-4">
...
// ─── Bookings Tab ────────────────────────────────────────────────
function BookingsTab({ adminEmail }) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Quản lý lịch hẹn</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Xem và cập nhật tất cả lịch tư vấn từ khách hàng</p>
                </div>
            ...
            <div className="flex gap-2 bg-white dark:bg-dark-900 p-1.5 rounded-xl border border-gray-200 dark:border-dark-800 w-fit">
                {[{ id: 'all', label: 'Tất cả' }, { id: 'pending', label: 'Chờ xác nhận' }, { id: 'confirmed', label: 'Đã xác nhận' }, { id: 'cancelled', label: 'Đã hủy' }]
                    .map(tab => (
                        <button key={tab.id} onClick={() => setFilter(tab.id)}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${filter === tab.id ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>
                            {tab.label}
                        </button>
                    ))}
            </div>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-dark-800">
                                <AnimatePresence>
                                    {bookings.map(booking => (
                                        <motion.tr key={booking.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                            ...
                                            <td className="px-6 py-4">
                                                ...
                                                {booking.email && <div className="text-gray-400 text-xs">{booking.email}</div>}
                                            </td>
                                            <td className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{formatDate(booking.date)}</td>
                                            <td className="px-6 py-4 text-gray-600 dark:text-gray-400 whitespace-nowrap">{TIME_LABELS[booking.time_slot] || booking.time_slot}</td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-2">
                                                    {booking.status !== 'confirmed' && (
                                                        <button onClick={() => handleStatusChange(booking.id, 'confirmed')} title="Xác nhận"
                                                    ...
                                                    {booking.status !== 'cancelled' && (
                                                        <button onClick={() => handleStatusChange(booking.id, 'cancelled')} title="Hủy lịch"
                                                    ...
                                                    {booking.status !== 'pending' && (
                                                        <button onClick={() => handleStatusChange(booking.id, 'pending')} title="Đặt lại về chờ"
            ...
            <p className="text-xs text-gray-400 text-center">
                Dữ liệu được cập nhật theo thời gian thực từ Supabase • {bookings.length} lịch hẹn đang hiển thị
            </p>
        </div>
    );
}
...
export default function Admin() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-dark-950 font-sans flex flex-col">
            {/* Header */}
            <header className="bg-white dark:bg-dark-900 border-b border-gray-200 dark:border-dark-800 sticky top-0 z-40">
                <div className="px-4 sm:px-6 h-16 flex items-center justify-between max-w-[1400px] mx-auto">
                    <div className="flex items-center gap-3">
                        {/* Mobile menu button */}
                        <button onClick={() => setSidebarOpen(o => !o)} className="lg:hidden w-9 h-9 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-dark-800 transition-colors">
                            <span className="material-icons text-gray-600 dark:text-gray-300">{sidebarOpen ? 'close' : 'menu'}</span>
                        </button>
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-sm text-gray-500 dark:text-gray-400 hidden sm:block">{adminEmail}</span>
                        <button onClick={handleSignOut} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 hover:text-red-500 transition-colors">
            ...
            <div className="flex flex-1 max-w-[1400px] mx-auto w-full">
                ...
                <>
                    ...
                    <aside className={`
                        fixed top-16 left-0 z-30 h-[calc(100vh-4rem)] bg-white dark:bg-dark-900 border-r border-gray-200 dark:border-dark-800 w-56 shrink-0
                        transition-transform duration-300 lg:relative lg:top-0 lg:translate-x-0 lg:h-auto
                        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
                    `}>
                        <nav className="p-4 space-y-1 sticky top-6">
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            {activeTab === 'testimonials' && (
                                <div className="space-y-2">
                                    <div className="mb-6">
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Thêm, sửa, ẩn/hiện đánh giá khách hàng hiển thị trên trang chủ.</p>
                                    </div>
                                    <TestimonialEditor />
                                </div>
                            )}
                            {activeTab === 'pricing' && (
                                <div className="space-y-2">
                                    <div className="mb-6">
                                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Quản lý bảng giá</h1>
                                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Điều chỉnh loại dự án, phong cách và gói dịch vụ trong công cụ tính giá.</p>
                                    </div>
                                    <PricingEditor />
                                </div>
                            )}
