import React from 'react';

export default function Services() {
    return (
        <section className="py-24 bg-gray-50 dark:bg-gray-800" id="services">
            <div className="container mx-auto px-6">
                <div className="text-center max-w-2xl mx-auto mb-16">
                    <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Our Services</h2>
                    <p className="text-gray-600 dark:text-gray-400">
                        From initial concept to final installation, we offer a comprehensive suite of design services tailored to your needs.
                    </p>
                </div>
                <div className="grid md:grid-cols-3 gap-8 text-center">
                    <div className="bg-white dark:bg-background-dark p-8 rounded-lg shadow-md">
                        <div className="bg-primary/10 text-primary w-16 h-16 rounded-full inline-flex items-center justify-center mb-6">
                            <span className="material-icons text-3xl">space_dashboard</span>
                        </div>
                        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Interior Design</h3>
                        <p className="text-gray-600 dark:text-gray-400">
                            Holistic space planning, material selection, and lighting design to create a cohesive environment.
                        </p>
                    </div>
                    <div className="bg-white dark:bg-background-dark p-8 rounded-lg shadow-md">
                        <div className="bg-primary/10 text-primary w-16 h-16 rounded-full inline-flex items-center justify-center mb-6">
                            <span className="material-icons text-3xl">chair</span>
                        </div>
                        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Custom Furniture</h3>
                        <p className="text-gray-600 dark:text-gray-400">
                            Bespoke furniture pieces designed and crafted to fit your space and style perfectly.
                        </p>
                    </div>
                    <div className="bg-white dark:bg-background-dark p-8 rounded-lg shadow-md">
                        <div className="bg-primary/10 text-primary w-16 h-16 rounded-full inline-flex items-center justify-center mb-6">
                            <span className="material-icons text-3xl">construction</span>
                        </div>
                        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Project Management</h3>
                        <p className="text-gray-600 dark:text-gray-400">
                            End-to-end oversight to ensure your project is completed on time, on budget, and to our high standards.
                        </p>
                    </div>
                </div>
            </div>
        </section>
    );
}
